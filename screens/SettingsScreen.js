...
import { getOpenAIApiKey, verifyOpenAIApiKey } from "../utils/apiConfig"
...
const SettingsScreen = ({ navigation }) => {
  ...
  // Add this function after the other useEffect hooks in SettingsScreen.js
  const testApiKey = async () => {
    try {
      const key = await getOpenAIApiKey()
      if (!key) {
        console.log("No API key found")
        Alert.alert("API Key Missing", "No OpenAI API key found. Please add your API key in the settings.")
        return
      }
      ...
      if (result.valid) {
        ...
        setApiKeyStatus({ valid: true, message: result.message })
        Alert.alert("API Key Valid", "Your OpenAI API key is working correctly!")
      } else {
        console.error("API key validation failed:", result.error)
        setApiKeyStatus({ valid: false, error: result.error })
        Alert.alert(
          "API Key Error",
          `Your OpenAI API key is not working: ${result.error}. Please check your key and try again.`,
        )
      }
    } catch (error) {
      console.error("Error testing API key:", error)
      setApiKeyStatus({ valid: false, error: error.message || "An unexpected error occurred" })
    }
  }
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        <GlassmorphicCard style={styles.section}>
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              ...
              Alert.alert("Privacy Policy", "You will be redirected to betteruai.com to view our Privacy Policy.", [
                { text: "Cancel", style: "cancel" },
                {
                  text: "Open Website",
                  onPress: () => {
                    try {
                      Linking.openURL("https://betteruai.com/privacy")
                    } catch (error) {
        ...
        <GlassmorphicCard style={styles.section}>
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              // Open the website in browser
              Alert.alert("Terms of Service", "You will be redirected to betteruai.com to view our Terms of Service.", [
                { text: "Cancel", style: "cancel" },
                {
                  text: "Open Website",
                  onPress: () => {
                    try {
                      Linking.openURL("https://betteruai.com")
                    } catch (error) {
                      Alert.alert("Error", "Could not open the website. Please try again later.")
                    }
                  },
        ...
        <GlassmorphicCard style={styles.section}>
          ...
          <TouchableOpacity style={styles.settingButton} onPress={testApiKey}>
            <Text style={styles.settingButtonText}>Test API Key</Text>
            <Ionicons name="chevron-forward" size={20} color="#aaa" />
          </TouchableOpacity>
          {apiKeyStatus && (
            <View style={styles.apiKeyStatusContainer}>
              <Text style={[styles.apiKeyStatusText, apiKeyStatus.valid ? styles.apiKeyValid : styles.apiKeyInvalid]}>
