...
const WorkoutScreen = ({ navigation, route }) => {
  ...
  const getFilteredWorkouts = () => {
    // Filter out the training plans that are in the Training Plans section
    let filteredStyles = trainingStyles.filter((style) => !["ppl", "upperlower", "5daybodypart"].includes(style.id))
  ...
  const renderWorkoutCard = ({ item }) => {
    // Calculate estimated calories for the workout
    const estimatedCalories = () => {
      // Base calorie burn rate per minute (varies by training style)
      ...
      // Extract minutes from duration (e.g., "45-60 min" -> 52.5)
      const durationMatch =
        item.duration && typeof item.duration === "string" ? item.duration.match(/(\d+)(?:-(\d+))?\s*min/) : null
      let minutes = 45 // Default

      if (durationMatch) {
        if (durationMatch[2]) {
          // Range format (e.g., "45-60 min")
          minutes = (Number.parseInt(durationMatch[1]) + Number.parseInt(durationMatch[2])) / 2
        } else {
          // Single value format (e.g., "45 min")
          minutes = Number.parseInt(durationMatch[1])
        }
      }
    ...
    return (
      ...
      >
        ...
        <View style={styles.workoutMeta}>
          ...
          {showCalories && (
            <View style={styles.metaItem}>
              <Ionicons name="flame-outline" size={18} color="#aaa" />
              <Text style={styles.metaText}>~{estimatedCalories()} cal</Text>
            </View>
          )}
        </View>
